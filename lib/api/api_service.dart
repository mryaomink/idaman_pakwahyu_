...
class APIService {
          ...
          for (var i = 0; i < it.data.length; i++) {
            umkm.add(
              new Umkm(
                  ...
                  jarak_km: double.parse(it.data[i]["jarak_km"]),
                  ...
                  aset_tetap: it.data[i]["aset_tetap"].toString().length > 0 ? double.parse(it.data[i]["aset_tetap"]) : 0,
                  omset: double.parse(it.data[i]["omset"]),
                  update_omset: it.data[i]["update_omset"].toString(),
                j_kary_p: int.parse(it.data[i]["j_kary_p"]),
                j_kary_l: int.parse(it.data[i]["j_kary_l"]),
              )
            );
          }
          return umkm;
          ...
          for (var i = 0; i < it.data.length; i++) {
            pasar.add(
                new PasarModel(
                    ...
                    lat: it.data[i]["lat"].toString(),
                    lon: it.data[i]["lon"].toString(),
                    alamat: it.data[i]["alamat"].toString(),
  ...
  Future<PostResponseModel> addKeranjang(KeranjangPostModel postModel) async {
    ...
    final response = await http.post(url, body: postModel.toJson());
    if (response.statusCode == 200 || response.statusCode == 400) {
  ...
  Future<StatsResposeModel> getStats() async {
    ...
    final response = await http.post(url, body: tpm.toJson());
    if (response.statusCode == 200 || response.statusCode == 400) {
  ...
  Future<ResponseModel> sendOtp(String nik, String no_telp, String nama_lgkp,String password, String otp) async {
    ...
    return ResponseModel(api_status: 1, api_message: "request pass !", api_authorization: "", data: []);
  }

  Future<PostResponseModel> updateProfil(ProfilPostModel postModel) async {
  ...
  Future<SingleResponseModel> cekNik(String nik) async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 400) {
        ...
        return SingleResponseModel(api_status: 0, api_message: "request gagal !", api_authorization: "", id: "", value: "");
        throw Exception('Failed to load data!');
      }
    }catch (e){
      print(e.toString());
    ...
    return SingleResponseModel(api_status: 0, api_message: "request failed !", api_authorization: "", id: "", value: "");;
  }
